...
DexScreener's priceChange buckets (see api.cards.price_points for the honest-approximation
...
`deps.get_dex()` is resolved via the module (`from api import deps`), never
`from api.deps import get_dex`, so tests can monkeypatch `deps.get_dex`.
"""
...
from api.cards import price_points
