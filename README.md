...
| **Status** | Phase A + B complete · **470 tests green in CI** (313 backend + 157 frontend) · deployable on Alibaba Cloud ECS · agent verified end-to-end |
...
>
> Recalling that deployer's rug history from memory takes **~0.8 ms**; re-deriving it cold by scanning all 13 mints on-chain takes **~175.5 s** — **≈220,209× faster** (~5 orders of magnitude; recall is single-digit ms, the cold path is network-bound and run-variable), and the cold path is exactly the work memory lets you skip.
